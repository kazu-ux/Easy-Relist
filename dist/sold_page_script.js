...
async function getBase64(imageUrls) {
    const imageBase64s = [];
...
async function setProduct() {
    const product = {
        ...
        itemCondition: document.querySelector('[data-testid="商品の状態"]')?.textContent ?? '',
        name: document
        ...
        shippingPayer: document.querySelector('[data-testid="配送料の負担"]')?.textContent ?? '',
        shippingMethod: document.querySelector('[data-testid="配送の方法"]')?.textContent ?? '',
        shippingFromArea: document.querySelector('[data-testid="発送元の地域"]')?.textContent ?? '',
        shippingDuration: document.querySelector('[data-testid="発送までの日数"]')?.textContent ??
            '',
        price: document.querySelector('[data-testid="price"]')?.getAttribute('value') ??
            '',
    };
    return product;
}
